...
```
**Expected deltas (tolerated):** Credit Note tax-sign discrepancies (the double-taxation bug — documented in Rule 1). These are known and corrected by `vw_clean_transactions`. A specific and separately-tracked sub-case of this bucket is Rule 16 (DK-593) below — same symptom, different mechanism.
...
## 16. DK-593: Gross-in-Excl Credit Note Header Defect

**Discovered during:** original DK-593 code-level detection, `src/lib/erpImportEngine.ts` (date unknown, predates this doctrine file). **Formalized as a numbered rule and its detection bug fixed** during PDP-34 (TWK002, 2026-09-13) — this pattern lived only as a code comment until then, which is exactly the kind of gap this doctrine file exists to close.

**Problem:** Certain ERP export batches — confirmed named: `DRTX2024.TXT`, `DRTX2025.TXT`, `DTRX2603.TXT` — populate a Credit Note's `transaction_headers.amount_excl` with the VAT-**inclusive** (gross) figure instead of the ex-VAT (net) figure, while `tax_amount` stays correct. Any query that naively sums `amount_excl + tax_amount` (the Rule 6 cross-check formula, or any account-level header rollup) double-counts VAT for these specific rows, producing an inflated header total.

**Two distinct manifestations, same root cause:**
1. **Single bad row.** The doc was only ever imported from one of the three affected batches. The item-level total (computed via the Rule 1 sign-corrected formula) ties *exactly* to `amount_excl` alone — never to `amount_excl + tax_amount`. Confirmed on 21 TWK002 docs (PDP-34): `7848, 8120, 8324, 8530, 8700, 8913, 9158, 9393, 9426, 9769, 10189, 10617, 10723, 11003, 11320, 11475, 11648, 11743, 11821, 11953, 14910`.
2. **Two header rows for the same doc.** The bad row from an affected batch was never rejected (see the code bug below), and a *later* re-import under a different, unaffected batch (e.g. `DETRANS2307.TXT`) added a second, correctly-valued row for the same `doc_no` — because the two rows' `amount_excl` genuinely differ, they hash to different fingerprints and both survive the `onConflict: 'fingerprint'` upsert; this is a real second value, not a PDP-31-style fingerprint-drift duplicate of identical data. Confirmed on 30 more TWK002 docs (PDP-34): `12131, 12207, 12214, 12215, 12329, 12494, 12502, 12550, 12724, 12825, 12908, 12958, 12982, 13019, 13057, 13101, 13235, 13335, 13500, 13587, 13716, 13874, 14034, 14232, 14237, 14414, 14554, 14649, 14711, 14820`. Any account-level header rollup that sums *every* header row for a doc_no (rather than picking one) will overstate this doc's contribution — this is the "chained same-day tax" pattern: the second row's `amount_excl` equals the first row's true net total (`amount_excl_1 + tax_amount`), because the first row was gross and the second is the corrected net.

**Code-level detection existed but was broken.** `ERPImportEngine.validateIntegrity()` (`src/lib/erpImportEngine.ts`) has always had a check meant to flag exactly this shape as `CRITICAL` (rejecting the row before insert) rather than a mere `WARNING`. The check computed `grossImpliedTax` from the header's own (signed) `amount_excl` — negative for every real-world Credit Note — and compared it against `actualTax`, which is forced positive via `Math.abs()`. Comparing a negative-valued and a positive-valued quantity for near-equality can never succeed, so `isGrossInExclDefect` was `false` for every real Credit Note ever imported, and the defective row was silently *accepted* as a mere `WARNING` in every case. **Fixed 2026-09-13:** the comparison now takes `Math.abs()` of both sides. Regression test: `src/lib/erpImportEngine.test.ts`, "flags CRITICAL for a real-world negative-amount Credit Note (PDP-34 sign-bug regression)" — uses TWK002 doc `00007848`'s real numbers and would have failed under the old code.

**What this means going forward:** any *new* import of a row from one of the three named batches (or any future batch with the same defect shape) is now correctly rejected as `CRITICAL` rather than silently accepted. It does **not** retroactively fix rows already in the database — the 21+30 TWK002 docs above (and any equivalent on other accounts, not yet swept) still carry the defect as of this writing. Per Rule 6, an *unresolved* single-bad-row case is a tolerated, expected delta — do not "fix" it by mutating `amount_excl` (the item-level total, via Rule 1, is already the trustworthy number for that doc). The two-header-row case needs judgment: any code that sums header rows per `doc_no` should prefer the later/corrected row rather than summing both.

**Cross-account recurrence:** portfolio-wide, not TWK002-specific. **335 distinct accounts** have at least one `Crd Note` header row sourced from one of the three named batches (`DRTX2024.TXT`, `DRTX2025.TXT`, `DTRX2603.TXT`) — every one of those rows could never have been correctly flagged `CRITICAL` before the 2026-09-13 sign-bug fix, since the detector was broken for every real (negative-amount) Credit Note, not just TWK002's. This scale was not present in the original PDP-32/PDP-34 investigation (which was scoped to TWK002 only) — a portfolio-wide sweep to characterize which of these 335 accounts actually carry the gross-in-excl symptom (vs. merely having some row from these files) is tracked as a follow-up, not yet done.
