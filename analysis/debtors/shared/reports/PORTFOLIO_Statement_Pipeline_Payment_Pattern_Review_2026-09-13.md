...
| Per-invoice, LIFO-batch — **asserted, unconfirmed** | One payment spans N invoices, newest first | JEN001 (see §5 — carry the caveat in the label itself) |
...
- **Part 3** — adopt the two-axis (`evidence_tier` × `settlement_unit`) taxonomy instead of the flat `matched_via` enum; correct the JIM001-FIFO/JEN001-LIFO conflation and carry JEN001's "asserted not proven" status explicitly wherever its pattern is cited; treat BU0002 as uncharacterized until its statement TXT is ingested, not as JIM001-by-analogy; apply the `monthly-batch-erp-bridge-reconciliation` exact-sum method to JIM001 as originally earmarked, rather than continuing to rely on the older tolerance-based reports for years without human review.
- **Part 4** — extend `portfolio_candidates.csv` (or a companion file keyed the same way) with the columns above, generating `remittance_basis` from `debtors:tag-check` rather than hand-entering it, and using an explicit "uncharacterized" state instead of a forced guess wherever evidence doesn't support a bucket yet.
