...
| `NO_ANSWER` | "no answer", "try again tomorrow" | **changed 2026-09-14** (was `predicted_due_date` = tomorrow, which removed the row from today's queue entirely). Queue row stays `PENDING`; `predicted_due_date` = **today** — the row stays in today's list instead of disappearing until tomorrow. What actually moves it is `fetchDesk`'s ordering: `ORDER BY predicted_due_date ASC, updated_at ASC` (added the same day) — a row nobody's called yet today keeps an older `updated_at` and sorts first; `touchQueue`'s `updated_at = now()` pushes a just-attempted row to the back of today's list instead. Requested directly: "push no answer to the back of the queue for that same day." |
