...
| **Phase 3a — context bootstrapping** | A fresh chat session with only DB access has none of this document's context (the intent table, the LPG-vs-deposit filter, the one-at-a-time convention) — that knowledge has to travel with the *data*, not live only in a markdown file the session can't read. Two pieces: (1) a claude.ai **Project** whose custom instructions are a condensed operator runbook, so every new chat in that Project inherits it automatically (parallel to how a Claude Code session auto-loads `CLAUDE.md`); (2) a `SOLICITATION_RULEBOOK` row in the existing `app_config` table (jsonb, already used this way for `RECON_SCORING_WEIGHTS` in the debtors domain) holding the actual intent table + query rules, with the Project instructions telling the session to read it first. That way the rulebook stays correct even from a client other than claude.ai, and updating it is a data change, not a re-onboarding exercise. |
